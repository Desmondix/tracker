...
// Funzione per aggiornare il cookie
...
function updateEventButtons() {
    ...
    const allEvents = [
        { name: 'Uscita dal laboratorio', points: 10 },
        { name: 'Sconfitta Brock', points: 80 },
        { name: 'Monte Luna', points: 30, specialEvent: true },
        { name: 'Sconfiggi rivale 1', points: 20 },
        { name: 'Torre di Lavandonia', points: 30 }
        // Aggiungi altri eventi qui se necessario
    ];
...
function createEventButton(event, redeemed = false) {
    ...
    if (event.specialEvent) {
        button.textContent = `${event.name} (+${event.points} punti)`;
        button.onclick = function () {
            showMonteLunaPrompt(event.points);
        };
    } else {
        button.textContent = `${event.name} (+${event.points} punti)`;
        button.onclick = function () {
...
function showMonteLunaPrompt(points) {
    const message = 'Hai fatto full clear di Monte Luna?';
    showConfirmationPrompt(message, function () {
        addPoints(points * 2, 'Monte Luna'); // Aggiungi 30 punti extra se full clear
    });
}
